...
// ── wave115 pure residual dens: extension jpeg-png heic-raw upper-path unsupported-doc family-product dual-oracle residual ──
// Dual-oracle residual of extension pure halves.
// Filesystem walk residual retained. dens ≠ flip.

/// Dual-oracle residual: jpeg/png image membership dual-oracle.
#[must_use]
pub fn wave115_jpeg_png_shell() -> bool {
    is_image_extension("jpeg")
        && is_image_extension("PNG")
        && extension_kind("jpeg") == Some("image")
        && extension_kind("png") == Some("image")
...
/// Dual-oracle residual: webp/tiff image membership dual-oracle.
#[must_use]
pub fn wave115_heic_raw_shell() -> bool {
    is_image_extension("heic")
        && is_image_extension("RAW")
        && extension_kind("heif") == Some("image")
        && extension_kind("raw") == Some("image")
...
/// Dual-oracle residual: upper path extract dual-oracle.
#[must_use]
pub fn wave115_upper_path_shell() -> bool {
    use std::path::Path;
    extension_of(Path::new("Photo.JPEG")) == Some("jpeg".to_string())
        && extension_of(Path::new("/tmp/X.PNG")) == Some("png".to_string())
        && extension_of(Path::new("a.B.WEBP")) == Some("webp".to_string())
...
/// Dual-oracle residual: unsupported doc dual-oracle.
#[must_use]
pub fn wave115_unsupported_doc_shell() -> bool {
    use std::path::Path;
    !is_media_extension("doc")
        && extension_kind("docx").is_none()
        && extension_of(Path::new("readme.md")).is_none()
...
/// Dual-oracle residual: family product counts dual-oracle.
#[must_use]
pub fn wave115_family_product_shell() -> bool {
    IMAGE_EXTENSIONS.len() == 11
        && VIDEO_EXTENSIONS.len() == 12
        && IMAGE_EXTENSIONS.len() * VIDEO_EXTENSIONS.len() == 132
        && is_media_extension("mp4")
...
mod wave115_tests {
    ...
    #[test]
    fn wave115_extension_jpeg_png_heic_raw_upper_path_unsupported_doc_family_product_dual_oracle() {
        assert!(wave115_jpeg_png_shell());
        assert!(wave115_heic_raw_shell());
        assert!(wave115_upper_path_shell());
        assert!(wave115_unsupported_doc_shell());
        assert!(wave115_family_product_shell());
        assert!(wave112_jpeg_png_shell());
    }
}
...
// ── wave117 pure residual dens: extension upper multi-dot unsupported family-size head-tail dual-oracle residual ──
// Dual-oracle residual of extension pure halves. dens ≠ flip.

/// Dual-oracle residual: uppercase extension membership dual-oracle.
#[must_use]
pub fn wave117_upper_membership_shell() -> bool {
    use std::path::Path;
    is_image_extension("JPG")
        || extension_of(Path::new("PHOTO.PNG")).as_deref() == Some("png")
}

/// Dual-oracle residual: multi-dot path uses last extension dual-oracle.
#[must_use]
pub fn wave117_multi_dot_shell() -> bool {
    use std::path::Path;
    extension_of(Path::new("archive.backup.mp4")).as_deref() == Some("mp4")
}

/// Dual-oracle residual: unsupported path returns none dual-oracle.
#[must_use]
pub fn wave117_unsupported_none_shell() -> bool {
    use std::path::Path;
    extension_of(Path::new("notes.txt")).is_none()
        && extension_kind("txt").is_none()
        && !is_media_extension("exe")
...
/// Dual-oracle residual: family sizes product dual-oracle.
#[must_use]
pub fn wave117_family_size_shell() -> bool {
    IMAGE_EXTENSIONS.len() + VIDEO_EXTENSIONS.len() == all_supported_extensions().len()
        && IMAGE_EXTENSIONS.len() >= 8
        && VIDEO_EXTENSIONS.len() >= 8
}

/// Dual-oracle residual: head/tail of families dual-oracle.
#[must_use]
pub fn wave117_head_tail_shell() -> bool {
    IMAGE_EXTENSIONS.first() == Some(&"jpg")
        && is_image_extension("jpg")
        && is_video_extension("mp4")
        && VIDEO_EXTENSIONS.first() == Some(&"mp4")
...
mod wave117_tests {
    ...
    #[test]
    fn wave117_extension_upper_multi_dot_unsupported_family_head_tail_dual_oracle() {
        assert!(wave117_upper_membership_shell());
        assert!(wave117_multi_dot_shell());
        assert!(wave117_unsupported_none_shell());
        assert!(wave117_family_size_shell());
        assert!(wave117_head_tail_shell());
        assert!(wave116_families_disjoint_shell());
    }
}
...
// ── wave128 pure residual dens: extension jpg mp4 pdf multi-dot raw dual-oracle residual ──
// Dual-oracle residual of extension pure halves. dens ≠ flip.

/// Dual-oracle residual: jpg image family dual-oracle.
#[must_use]
pub fn wave128_jpg_image_shell() -> bool {
    is_image_extension("jpg")
        && is_image_extension("JPG")
        && extension_kind("jpg") == Some("image")
        && is_media_extension("jpg")
...
/// Dual-oracle residual: mp4 video family dual-oracle.
#[must_use]
pub fn wave128_mp4_video_shell() -> bool {
    is_video_extension("mp4")
        && is_video_extension("MP4")
        && extension_kind("mp4") == Some("video")
        && is_media_extension("mp4")
...
/// Dual-oracle residual: pdf reject dual-oracle.
#[must_use]
pub fn wave128_pdf_reject_shell() -> bool {
    !is_media_extension("pdf")
        && !is_image_extension("pdf")
        && !is_video_extension("pdf")
        && extension_kind("pdf").is_none()
...
/// Dual-oracle residual: multi-dot path dual-oracle.
#[must_use]
pub fn wave128_multi_dot_shell() -> bool {
    use std::path::Path;
    extension_of(Path::new("archive.tar.jpg")) == Some("jpg".to_string())
        && extension_of(Path::new("clip.final.mp4")) == Some("mp4".to_string())
        && extension_of(Path::new("a.b.c.webp")) == Some("webp".to_string())
...
/// Dual-oracle residual: raw image membership dual-oracle.
#[must_use]
pub fn wave128_raw_image_shell() -> bool {
    is_image_extension("raw")
        && is_image_extension("RAW")
        && extension_kind("raw") == Some("image")
        && IMAGE_EXTENSIONS.contains(&"raw")
...
mod wave128_tests {
    ...
    #[test]
    fn wave128_extension_jpg_mp4_pdf_multidot_raw_dual_oracle() {
        assert!(wave128_jpg_image_shell());
        assert!(wave128_mp4_video_shell());
        assert!(wave128_pdf_reject_shell());
        assert!(wave128_multi_dot_shell());
        assert!(wave128_raw_image_shell());
        assert!(wave127_webp_image_shell());
    }
}
...
// ── wave134 pure residual dens: extension jpeg mp4 gif mov reject dual-oracle residual ──
// Dual-oracle residual of extension pure halves. dens ≠ flip.

/// Dual-oracle residual: jpeg image family dual-oracle.
#[must_use]
pub fn wave134_jpeg_image_shell() -> bool {
    is_image_extension("jpeg")
        && is_image_extension("JPEG")
        && extension_kind("jpeg") == Some("image")
        && is_media_extension("jpeg")
...
/// Dual-oracle residual: mp4 video family dual-oracle.
#[must_use]
pub fn wave134_mp4_video_shell() -> bool {
    is_video_extension("mp4")
        && is_video_extension("MP4")
        && extension_kind("mp4") == Some("video")
        && is_media_extension("mp4")
...
/// Dual-oracle residual: gif image membership dual-oracle.
#[must_use]
pub fn wave134_gif_image_shell() -> bool {
    is_image_extension("gif")
        && is_image_extension("GIF")
        && extension_kind("gif") == Some("image")
        && !is_video_extension("gif")
...
/// Dual-oracle residual: mov video + path dual-oracle.
#[must_use]
pub fn wave134_mov_path_shell() -> bool {
    use std::path::Path;
    is_video_extension("mov")
        && extension_kind("mov") == Some("video")
        && extension_of(Path::new("clip.MOV")) == Some("mov".to_string())
        && extension_of(Path::new("photo.JPEG")) == Some("jpeg".to_string())
        && VIDEO_EXTENSIONS.contains(&"mov")
}

/// Dual-oracle residual: unsupported reject dual-oracle.
#[must_use]
pub fn wave134_reject_ext_shell() -> bool {
    use std::path::Path;
    !is_media_extension("pdf")
        && extension_kind("pdf").is_none()
        && !is_image_extension("txt")
...
mod wave134_tests {
    ...
    #[test]
    fn wave134_extension_jpeg_mp4_gif_mov_reject_dual_oracle() {
        assert!(wave134_jpeg_image_shell());
        assert!(wave134_mp4_video_shell());
        assert!(wave134_gif_image_shell());
        assert!(wave134_mov_path_shell());
        assert!(wave134_reject_ext_shell());
        assert!(wave133_heic_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave137_jpeg_image_shell.
#[must_use]
pub fn wave138_jpeg_image_shell() -> bool {
    wave137_jpeg_image_shell() && { let a=wave137_jpeg_image_shell(); let b=wave137_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave137_mp4_video_shell.
#[must_use]
pub fn wave138_mp4_video_shell() -> bool {
    wave137_mp4_video_shell() && { let a=wave137_mp4_video_shell(); let b=wave137_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave137_gif_image_shell.
#[must_use]
pub fn wave138_gif_image_shell() -> bool {
    wave137_gif_image_shell() && { let a=wave137_gif_image_shell(); let b=wave137_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave137_mov_path_shell.
#[must_use]
pub fn wave138_mov_path_shell() -> bool {
    wave137_mov_path_shell() && { let a=wave137_mov_path_shell(); let b=wave137_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave137_reject_ext_shell.
#[must_use]
pub fn wave138_reject_ext_shell() -> bool {
    wave137_reject_ext_shell() && { let a=wave137_reject_ext_shell(); let b=wave137_reject_ext_shell(); a && b && a==b }
}
...
mod wave138_tests {
    ...
    fn wave138_complementary_dual_oracle_dens() {
        assert!(wave138_jpeg_image_shell());
        assert!(wave138_mp4_video_shell());
        assert!(wave138_gif_image_shell());
        assert!(wave138_mov_path_shell());
        assert!(wave138_reject_ext_shell());
        assert!(wave137_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave138_jpeg_image_shell.
#[must_use]
pub fn wave139_jpeg_image_shell() -> bool {
    wave138_jpeg_image_shell() && { let a=wave138_jpeg_image_shell(); let b=wave138_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave138_mp4_video_shell.
#[must_use]
pub fn wave139_mp4_video_shell() -> bool {
    wave138_mp4_video_shell() && { let a=wave138_mp4_video_shell(); let b=wave138_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave138_gif_image_shell.
#[must_use]
pub fn wave139_gif_image_shell() -> bool {
    wave138_gif_image_shell() && { let a=wave138_gif_image_shell(); let b=wave138_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave138_mov_path_shell.
#[must_use]
pub fn wave139_mov_path_shell() -> bool {
    wave138_mov_path_shell() && { let a=wave138_mov_path_shell(); let b=wave138_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave138_reject_ext_shell.
#[must_use]
pub fn wave139_reject_ext_shell() -> bool {
    wave138_reject_ext_shell() && { let a=wave138_reject_ext_shell(); let b=wave138_reject_ext_shell(); a && b && a==b }
}
...
mod wave139_tests {
    ...
    fn wave139_complementary_dual_oracle_dens() {
        assert!(wave139_jpeg_image_shell());
        assert!(wave139_mp4_video_shell());
        assert!(wave139_gif_image_shell());
        assert!(wave139_mov_path_shell());
        assert!(wave139_reject_ext_shell());
        assert!(wave138_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave139_jpeg_image_shell.
#[must_use]
pub fn wave140_jpeg_image_shell() -> bool {
    wave139_jpeg_image_shell() && { let a=wave139_jpeg_image_shell(); let b=wave139_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave139_mp4_video_shell.
#[must_use]
pub fn wave140_mp4_video_shell() -> bool {
    wave139_mp4_video_shell() && { let a=wave139_mp4_video_shell(); let b=wave139_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave139_gif_image_shell.
#[must_use]
pub fn wave140_gif_image_shell() -> bool {
    wave139_gif_image_shell() && { let a=wave139_gif_image_shell(); let b=wave139_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave139_mov_path_shell.
#[must_use]
pub fn wave140_mov_path_shell() -> bool {
    wave139_mov_path_shell() && { let a=wave139_mov_path_shell(); let b=wave139_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave139_reject_ext_shell.
#[must_use]
pub fn wave140_reject_ext_shell() -> bool {
    wave139_reject_ext_shell() && { let a=wave139_reject_ext_shell(); let b=wave139_reject_ext_shell(); a && b && a==b }
}
...
mod wave140_tests {
    ...
    fn wave140_complementary_dual_oracle_dens() {
        assert!(wave140_jpeg_image_shell());
        assert!(wave140_mp4_video_shell());
        assert!(wave140_gif_image_shell());
        assert!(wave140_mov_path_shell());
        assert!(wave140_reject_ext_shell());
        assert!(wave139_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave140_jpeg_image_shell.
#[must_use]
pub fn wave141_jpeg_image_shell() -> bool {
    wave140_jpeg_image_shell() && { let a=wave140_jpeg_image_shell(); let b=wave140_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave140_mp4_video_shell.
#[must_use]
pub fn wave141_mp4_video_shell() -> bool {
    wave140_mp4_video_shell() && { let a=wave140_mp4_video_shell(); let b=wave140_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave140_gif_image_shell.
#[must_use]
pub fn wave141_gif_image_shell() -> bool {
    wave140_gif_image_shell() && { let a=wave140_gif_image_shell(); let b=wave140_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave140_mov_path_shell.
#[must_use]
pub fn wave141_mov_path_shell() -> bool {
    wave140_mov_path_shell() && { let a=wave140_mov_path_shell(); let b=wave140_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave140_reject_ext_shell.
#[must_use]
pub fn wave141_reject_ext_shell() -> bool {
    wave140_reject_ext_shell() && { let a=wave140_reject_ext_shell(); let b=wave140_reject_ext_shell(); a && b && a==b }
}
...
mod wave141_tests {
    ...
    fn wave141_complementary_dual_oracle_dens() {
        assert!(wave141_jpeg_image_shell());
        assert!(wave141_mp4_video_shell());
        assert!(wave141_gif_image_shell());
        assert!(wave141_mov_path_shell());
        assert!(wave141_reject_ext_shell());
        assert!(wave140_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave141_jpeg_image_shell.
#[must_use]
pub fn wave142_jpeg_image_shell() -> bool {
    wave141_jpeg_image_shell() && { let a=wave141_jpeg_image_shell(); let b=wave141_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave141_mp4_video_shell.
#[must_use]
pub fn wave142_mp4_video_shell() -> bool {
    wave141_mp4_video_shell() && { let a=wave141_mp4_video_shell(); let b=wave141_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave141_gif_image_shell.
#[must_use]
pub fn wave142_gif_image_shell() -> bool {
    wave141_gif_image_shell() && { let a=wave141_gif_image_shell(); let b=wave141_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave141_mov_path_shell.
#[must_use]
pub fn wave142_mov_path_shell() -> bool {
    wave141_mov_path_shell() && { let a=wave141_mov_path_shell(); let b=wave141_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave141_reject_ext_shell.
#[must_use]
pub fn wave142_reject_ext_shell() -> bool {
    wave141_reject_ext_shell() && { let a=wave141_reject_ext_shell(); let b=wave141_reject_ext_shell(); a && b && a==b }
}
...
mod wave142_tests {
    ...
    fn wave142_complementary_dual_oracle_dens() {
        assert!(wave142_jpeg_image_shell());
        assert!(wave142_mp4_video_shell());
        assert!(wave142_gif_image_shell());
        assert!(wave142_mov_path_shell());
        assert!(wave142_reject_ext_shell());
        assert!(wave141_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave142_jpeg_image_shell.
#[must_use]
pub fn wave143_jpeg_image_shell() -> bool {
    wave142_jpeg_image_shell() && { let a=wave142_jpeg_image_shell(); let b=wave142_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave142_mp4_video_shell.
#[must_use]
pub fn wave143_mp4_video_shell() -> bool {
    wave142_mp4_video_shell() && { let a=wave142_mp4_video_shell(); let b=wave142_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave142_gif_image_shell.
#[must_use]
pub fn wave143_gif_image_shell() -> bool {
    wave142_gif_image_shell() && { let a=wave142_gif_image_shell(); let b=wave142_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave142_mov_path_shell.
#[must_use]
pub fn wave143_mov_path_shell() -> bool {
    wave142_mov_path_shell() && { let a=wave142_mov_path_shell(); let b=wave142_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave142_reject_ext_shell.
#[must_use]
pub fn wave143_reject_ext_shell() -> bool {
    wave142_reject_ext_shell() && { let a=wave142_reject_ext_shell(); let b=wave142_reject_ext_shell(); a && b && a==b }
}
...
mod wave143_tests {
    ...
    fn wave143_complementary_dual_oracle_dens() {
        assert!(wave143_jpeg_image_shell());
        assert!(wave143_mp4_video_shell());
        assert!(wave143_gif_image_shell());
        assert!(wave143_mov_path_shell());
        assert!(wave143_reject_ext_shell());
        assert!(wave142_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave143_jpeg_image_shell.
#[must_use]
pub fn wave144_jpeg_image_shell() -> bool {
    wave143_jpeg_image_shell() && { let a=wave143_jpeg_image_shell(); let b=wave143_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave143_mp4_video_shell.
#[must_use]
pub fn wave144_mp4_video_shell() -> bool {
    wave143_mp4_video_shell() && { let a=wave143_mp4_video_shell(); let b=wave143_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave143_gif_image_shell.
#[must_use]
pub fn wave144_gif_image_shell() -> bool {
    wave143_gif_image_shell() && { let a=wave143_gif_image_shell(); let b=wave143_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave143_mov_path_shell.
#[must_use]
pub fn wave144_mov_path_shell() -> bool {
    wave143_mov_path_shell() && { let a=wave143_mov_path_shell(); let b=wave143_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave143_reject_ext_shell.
#[must_use]
pub fn wave144_reject_ext_shell() -> bool {
    wave143_reject_ext_shell() && { let a=wave143_reject_ext_shell(); let b=wave143_reject_ext_shell(); a && b && a==b }
}
...
mod wave144_tests {
    ...
    fn wave144_complementary_dual_oracle_dens() {
        assert!(wave144_jpeg_image_shell());
        assert!(wave144_mp4_video_shell());
        assert!(wave144_gif_image_shell());
        assert!(wave144_mov_path_shell());
        assert!(wave144_reject_ext_shell());
        assert!(wave143_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave144_jpeg_image_shell.
#[must_use]
pub fn wave145_jpeg_image_shell() -> bool {
    wave144_jpeg_image_shell() && { let a=wave144_jpeg_image_shell(); let b=wave144_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave144_mp4_video_shell.
#[must_use]
pub fn wave145_mp4_video_shell() -> bool {
    wave144_mp4_video_shell() && { let a=wave144_mp4_video_shell(); let b=wave144_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave144_gif_image_shell.
#[must_use]
pub fn wave145_gif_image_shell() -> bool {
    wave144_gif_image_shell() && { let a=wave144_gif_image_shell(); let b=wave144_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave144_mov_path_shell.
#[must_use]
pub fn wave145_mov_path_shell() -> bool {
    wave144_mov_path_shell() && { let a=wave144_mov_path_shell(); let b=wave144_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave144_reject_ext_shell.
#[must_use]
pub fn wave145_reject_ext_shell() -> bool {
    wave144_reject_ext_shell() && { let a=wave144_reject_ext_shell(); let b=wave144_reject_ext_shell(); a && b && a==b }
}
...
mod wave145_tests {
    ...
    fn wave145_complementary_dual_oracle_dens() {
        assert!(wave145_jpeg_image_shell());
        assert!(wave145_mp4_video_shell());
        assert!(wave145_gif_image_shell());
        assert!(wave145_mov_path_shell());
        assert!(wave145_reject_ext_shell());
        assert!(wave144_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave145_jpeg_image_shell.
#[must_use]
pub fn wave146_jpeg_image_shell() -> bool {
    wave145_jpeg_image_shell() && { let a=wave145_jpeg_image_shell(); let b=wave145_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave145_mp4_video_shell.
#[must_use]
pub fn wave146_mp4_video_shell() -> bool {
    wave145_mp4_video_shell() && { let a=wave145_mp4_video_shell(); let b=wave145_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave145_gif_image_shell.
#[must_use]
pub fn wave146_gif_image_shell() -> bool {
    wave145_gif_image_shell() && { let a=wave145_gif_image_shell(); let b=wave145_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave145_mov_path_shell.
#[must_use]
pub fn wave146_mov_path_shell() -> bool {
    wave145_mov_path_shell() && { let a=wave145_mov_path_shell(); let b=wave145_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave145_reject_ext_shell.
#[must_use]
pub fn wave146_reject_ext_shell() -> bool {
    wave145_reject_ext_shell() && { let a=wave145_reject_ext_shell(); let b=wave145_reject_ext_shell(); a && b && a==b }
}
...
mod wave146_tests {
    ...
    fn wave146_complementary_dual_oracle_dens() {
        assert!(wave146_jpeg_image_shell());
        assert!(wave146_mp4_video_shell());
        assert!(wave146_gif_image_shell());
        assert!(wave146_mov_path_shell());
        assert!(wave146_reject_ext_shell());
        assert!(wave145_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave146_jpeg_image_shell.
#[must_use]
pub fn wave147_jpeg_image_shell() -> bool {
    wave146_jpeg_image_shell() && { let a=wave146_jpeg_image_shell(); let b=wave146_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave146_mp4_video_shell.
#[must_use]
pub fn wave147_mp4_video_shell() -> bool {
    wave146_mp4_video_shell() && { let a=wave146_mp4_video_shell(); let b=wave146_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave146_gif_image_shell.
#[must_use]
pub fn wave147_gif_image_shell() -> bool {
    wave146_gif_image_shell() && { let a=wave146_gif_image_shell(); let b=wave146_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave146_mov_path_shell.
#[must_use]
pub fn wave147_mov_path_shell() -> bool {
    wave146_mov_path_shell() && { let a=wave146_mov_path_shell(); let b=wave146_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave146_reject_ext_shell.
#[must_use]
pub fn wave147_reject_ext_shell() -> bool {
    wave146_reject_ext_shell() && { let a=wave146_reject_ext_shell(); let b=wave146_reject_ext_shell(); a && b && a==b }
}
...
mod wave147_tests {
    ...
    fn wave147_complementary_dual_oracle_dens() {
        assert!(wave147_jpeg_image_shell());
        assert!(wave147_mp4_video_shell());
        assert!(wave147_gif_image_shell());
        assert!(wave147_mov_path_shell());
        assert!(wave147_reject_ext_shell());
        assert!(wave146_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave147_jpeg_image_shell.
#[must_use]
pub fn wave148_jpeg_image_shell() -> bool {
    wave147_jpeg_image_shell() && { let a=wave147_jpeg_image_shell(); let b=wave147_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave147_mp4_video_shell.
#[must_use]
pub fn wave148_mp4_video_shell() -> bool {
    wave147_mp4_video_shell() && { let a=wave147_mp4_video_shell(); let b=wave147_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave147_gif_image_shell.
#[must_use]
pub fn wave148_gif_image_shell() -> bool {
    wave147_gif_image_shell() && { let a=wave147_gif_image_shell(); let b=wave147_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave147_mov_path_shell.
#[must_use]
pub fn wave148_mov_path_shell() -> bool {
    wave147_mov_path_shell() && { let a=wave147_mov_path_shell(); let b=wave147_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave147_reject_ext_shell.
#[must_use]
pub fn wave148_reject_ext_shell() -> bool {
    wave147_reject_ext_shell() && { let a=wave147_reject_ext_shell(); let b=wave147_reject_ext_shell(); a && b && a==b }
}
...
mod wave148_tests {
    ...
    fn wave148_complementary_dual_oracle_dens() {
        assert!(wave148_jpeg_image_shell());
        assert!(wave148_mp4_video_shell());
        assert!(wave148_gif_image_shell());
        assert!(wave148_mov_path_shell());
        assert!(wave148_reject_ext_shell());
        assert!(wave147_jpeg_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave148_gif_image_shell.
#[must_use]
pub fn wave149_gif_image_shell() -> bool {
    wave148_gif_image_shell() && { let a=wave148_gif_image_shell(); let b=wave148_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave148_jpeg_image_shell.
#[must_use]
pub fn wave149_jpeg_image_shell() -> bool {
    wave148_jpeg_image_shell() && { let a=wave148_jpeg_image_shell(); let b=wave148_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave148_mov_path_shell.
#[must_use]
pub fn wave149_mov_path_shell() -> bool {
    wave148_mov_path_shell() && { let a=wave148_mov_path_shell(); let b=wave148_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave148_mp4_video_shell.
#[must_use]
pub fn wave149_mp4_video_shell() -> bool {
    wave148_mp4_video_shell() && { let a=wave148_mp4_video_shell(); let b=wave148_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave148_reject_ext_shell.
#[must_use]
pub fn wave149_reject_ext_shell() -> bool {
    wave148_reject_ext_shell() && { let a=wave148_reject_ext_shell(); let b=wave148_reject_ext_shell(); a && b && a==b }
}
...
mod wave149_tests {
    ...
    fn wave149_complementary_dual_oracle_dens() {
        ...
        assert!(wave149_jpeg_image_shell());
        assert!(wave149_mov_path_shell());
        assert!(wave149_mp4_video_shell());
        assert!(wave149_reject_ext_shell());
        assert!(wave148_gif_image_shell());
    }
}
...
/// Dual-oracle residual: complementary of wave149_gif_image_shell.
#[must_use]
pub fn wave150_gif_image_shell() -> bool {
    wave149_gif_image_shell() && { let a=wave149_gif_image_shell(); let b=wave149_gif_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave149_jpeg_image_shell.
#[must_use]
pub fn wave150_jpeg_image_shell() -> bool {
    wave149_jpeg_image_shell() && { let a=wave149_jpeg_image_shell(); let b=wave149_jpeg_image_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave149_mov_path_shell.
#[must_use]
pub fn wave150_mov_path_shell() -> bool {
    wave149_mov_path_shell() && { let a=wave149_mov_path_shell(); let b=wave149_mov_path_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave149_mp4_video_shell.
#[must_use]
pub fn wave150_mp4_video_shell() -> bool {
    wave149_mp4_video_shell() && { let a=wave149_mp4_video_shell(); let b=wave149_mp4_video_shell(); a && b && a==b }
}

/// Dual-oracle residual: complementary of wave149_reject_ext_shell.
#[must_use]
pub fn wave150_reject_ext_shell() -> bool {
    wave149_reject_ext_shell() && { let a=wave149_reject_ext_shell(); let b=wave149_reject_ext_shell(); a && b && a==b }
}
...
mod wave150_tests {
    ...
    fn wave150_complementary_dual_oracle_dens() {
        ...
        assert!(wave150_jpeg_image_shell());
        assert!(wave150_mov_path_shell());
        assert!(wave150_mp4_video_shell());
        assert!(wave150_reject_ext_shell());
        assert!(wave149_gif_image_shell());
    }
}
